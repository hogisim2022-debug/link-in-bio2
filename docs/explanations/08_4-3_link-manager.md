# Link Manager (Drag & Drop)

**완료한 task:** `4. Admin 페이지 > 4-3. Link Manager`  
**수정 파일:** `components/admin/LinkEditor.tsx`, `components/admin/AdminClient.tsx`, `app/globals.css`

---

## 이 문서를 읽고 나면 풀 수 있어요

1. `@dnd-kit`에서 드래그 이벤트 감지는 `DndContext`가 담당한다.
2. 핸들 버튼에만 `listeners`를 붙이면 다른 버튼 클릭이 드래그와 충돌하지 않는다.
3. 드래그로 순서를 바꾸려면 드래그가 끝난 후 배열을 직접 재정렬해야 한다.

---

## 무엇을 했나?

Admin 설정 패널에 링크 목록이 나타나고, 드래그로 순서를 바꾸거나 눈 아이콘으로 노출을 토글하거나 삭제할 수 있다. 변경사항은 오른쪽 Live Preview에 즉시 반영된다.

```
┌──────────────────────────────────────────────┐
│ ⠿  카카오톡 문의           ✏  👁  🗑        │  ← 드래그 핸들 / 수정 / 노출 / 삭제
├──────────────────────────────────────────────┤
│ ⠿  이메일 문의             ✏  👁  🗑        │
├──────────────────────────────────────────────┤
│ ⠿  ~~강의 소개 페이지~~    ✏  🙈  🗑        │  ← 숨김 상태: 취소선 + 눈 감김
├──────────────────────────────────────────────┤
│ ⠿  AI 교육 블로그          ✏  👁  🗑        │
└──────────────────────────────────────────────┘
┌ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ┐
          +  링크 추가
└ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ┘
```

---

## 핵심 개념 1: @dnd-kit 구조

`@dnd-kit`은 세 레이어로 나뉜다.

```
DndContext          ← 드래그 이벤트 감지 (어디서 잡고, 어디에 놓았는지)
└── SortableContext ← 정렬 가능한 아이템 목록 관리
    └── useSortable ← 각 아이템에 드래그 능력 부여
```


## 핵심 개념 2: `arrayMove`로 순서 변경

드래그가 끝나면 `active.id`(집은 것)와 `over.id`(놓은 곳)를 이용해 배열을 재정렬한다.


---

## 핵심 개념 3: `useSortable`로 아이템에 드래그 부여

```
링크 행 전체     → 드래그 위치 추적
GripVertical ⠿  → 드래그 조작 (여기만 잡아야 드래그됨)
눈 버튼 / 삭제   → 드래그와 무관, 클릭만 동작
```

`listeners`를 전체 row가 아닌 핸들 버튼에만 붙이면, 버튼 클릭(눈, 삭제)이 드래그와 충돌하지 않는다.

- ❌ 과거: `"드래그 앤 드롭 붙여줘"`
- ✅ 현재: `"@dnd-kit으로 DndContext > SortableContext > useSortable 구조로 잡아줘. listeners는 GripVertical 핸들에만 붙여서 버튼 클릭이랑 충돌 안 나게 해줘"`

---


## 다음 단계

- 링크 추가 폼 구현
- 링크 수정 인라인 에디터
- Supabase 연동 후 `handleDragEnd`의 TODO에서 `order` 필드 일괄 UPDATE
- 삭제 시 TODO에서 DB DELETE 호출

---

## 이렇게 확인하세요

1. 터미널에서 `npm run dev` 실행
2. 브라우저에서 `http://localhost:3000/admin` 접속
3. 좌측 설정 패널에 링크 목록이 표시되는지 확인
4. ⠿ 핸들을 드래그해 순서를 바꾸고 오른쪽 미리보기에 즉시 반영되는지 확인
5. 👁 눈 아이콘 클릭 → 링크가 숨겨지고 미리보기에서 사라지는지 확인
6. 🗑 쓰레기통 아이콘 클릭 → 목록과 미리보기에서 제거되는지 확인

---

## 퀴즈 정답

1. `@dnd-kit`에서 드래그 이벤트 감지는 `DndContext`가 담당한다. → **O**  
   ↳ `DndContext` → `SortableContext` → `useSortable` 3단계 구조다.

2. 핸들 버튼에만 `listeners`를 붙이면 다른 버튼 클릭이 드래그와 충돌하지 않는다. → **O**  
   ↳ 행 전체에 붙이면 눈 버튼, 삭제 버튼을 클릭할 때도 드래그가 시작된다.

3. 드래그로 순서를 바꾸려면 드래그가 끝난 후 배열을 직접 재정렬해야 한다. → **O**  
   ↳ `arrayMove(배열, 원래인덱스, 새인덱스)`로 재정렬한다.
